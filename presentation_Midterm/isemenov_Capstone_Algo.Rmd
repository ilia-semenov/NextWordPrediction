...
trigramDTM<-function(corpus){
        ...
        dtm<-DocumentTermMatrix(corpus,control = list(tokenize = trigramTokenizer))
        mdtm<-as.matrix(dtm)
        return(list(dtm,mdtm))
}

ngramDTM<-function(corpus,n){
        ngramTokenizer <- function(x) NGramTokenizer(x, Weka_control(min = 1, max = n))
...
#clean
us_corpus_samp<-cleanCorpus(us_corpus)

us_corpus_samp[[1]][[1]][[16]]
...
four_dtm<-ngramDTM(us_corpus_samp,4)
f4<-colSums(four_dtm[[2]])

head(f1)

f1<-colSums(uni_dtm[[2]])
f2<-colSums(bi_dtm[[2]])
f3<-colSums(tri_dtm[[2]])


dt3<-data.table(w1=word(names(f3),1),
                w2=word(names(f3),2),
                w3=word(names(f3),3),freq=f3)

dt2<-data.table(w1=word(names(f2),1),
                w2=word(names(f2),2),freq=f2)

dt1<-unigramFreq(us_corpus_samp)
dt2<-ngramFreq(us_corpus_samp,2)
dt3<-ngramFreq(us_corpus_samp,3)
